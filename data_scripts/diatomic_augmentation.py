...
def parse_args():
    ...
    # Training results path (for filtering structures)
...
def setup_dataset_paths(dataset: str) -> tuple[str, str, tuple[str, ...]]:
    """Setup dataset-specific paths and properties."""
    if dataset == "Silica":
...
def main():
    ...
    if args.n_round_augmentation < 1:
        ...
        modified_atoms_list = [
            random_perturb(
                pull_atoms(
                    atoms, args.pull_max_strength[0], 'uniform', std=args.std, 
                    n_pairs_to_choose=args.n_pairs_to_choose
                ),
                min(args.n_perturb_sites, len(atoms)), args.perturb_max_strength[0], 
        ...
        for i in range(args.n_round_augmentation):
            ...
            round_modified = [
                random_perturb(
                    pull_atoms(
                        atoms, args.pull_max_strength[i], 'uniform', std=args.std,
                        n_pairs_to_choose=args.n_pairs_to_choose
                    ),
                    min(args.n_perturb_sites, len(atoms)), args.perturb_max_strength[i], 
                    'uniform', std=args.std
