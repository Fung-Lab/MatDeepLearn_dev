...
if __name__ == '__main__':
    ...
    for i in range(num_runs):
        ...
        command = "python scripts/main.py --run_mode=train --config_path=configs/ct_ft/masking15/config_Phonons_ct_ft.yml"
        subprocess.run(command, shell=True)

        # command = "python scripts/main.py --run_mode=train --config_path=configs/ct_ft/masking15/config_GVRH.yml"
        # subprocess.run(command, shell=True)

        command = "python scripts/main.py --run_mode=train --config_path=configs/ct_ft/masking15/config_GVRH_ct_ft.yml"
        subprocess.run(command, shell=True)

        # command = "python scripts/main.py --run_mode=train --config_path=configs/ct_ft/masking15/config_KVRH.yml"
        # subprocess.run(command, shell=True)

        command = "python scripts/main.py --run_mode=train --config_path=configs/ct_ft/masking15/config_KVRH_ct_ft.yml"
        subprocess.run(command, shell=True)
