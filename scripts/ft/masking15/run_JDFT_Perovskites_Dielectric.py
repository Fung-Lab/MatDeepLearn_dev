...
if __name__ == '__main__':
    ...
    for i in range(num_runs):
        ...
        command = "python scripts/main.py --run_mode=train --config_path=configs/ct_ft/masking15/config_JDFT_ct_ft.yml"
        subprocess.run(command, shell=True)

        # command = "python scripts/main.py --run_mode=train --config_path=configs/ct_ft/masking15/config_Dielectric.yml"
        # subprocess.run(command, shell=True)

        command = "python scripts/main.py --run_mode=train --config_path=configs/ct_ft/masking15/config_Dielectric_ct_ft.yml"
        subprocess.run(command, shell=True)

        # command = "python scripts/main.py --run_mode=train --config_path=configs/ct_ft/masking15/config_Perovskites.yml"
        # subprocess.run(command, shell=True)

        command = "python scripts/main.py --run_mode=train --config_path=configs/ct_ft/masking15/config_Perovskites_ct_ft.yml"
        subprocess.run(command, shell=True)
