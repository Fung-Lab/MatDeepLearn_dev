...
def langevin_random_force(pos: torch.Tensor, temperature: float, dt:float, gamma:float):
    """
    Generate a random force vector for each atom for Langevin dynamics.

    Parameters:
    - pos: tensor of particle positions
    - temperature: simulation temperature 
    - dt: timestep
    - gamma: friction coefficient
    
    Returns:
    - random_forces: random force vectors for each atom
    """
    num_particles, dim = pos.shape
    device = pos.device  
    sigma = math.sqrt(2 * gamma * temperature * dt)
    random_forces = torch.normal(0, sigma, size=(num_particles, dim), device=device)
    return random_forces

def step(pos, vels, forces, mass, dt):
    """
    One step of simulation

    Parameters:
    - pos: tensor of particle positions
    - vels: velocities of atoms
    - forces: tensor of forces of atoms
    - mass: mass of atoms
    - dt: timestep
...
def simulation(file_path, config, steps, log_per, learning_rate, num_structures=-1, batch_size=4, device='cpu'):
    ...
    for i in range(len(loader_iter)):
        ...
        print(langevin_random_force(pos, 1000, 0.5, 0.1).shape)
