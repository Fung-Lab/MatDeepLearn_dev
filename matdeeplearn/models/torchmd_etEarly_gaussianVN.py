...
class GaussianVN(torch.nn.Module):
    def __init__(self, pos, num_gaussians=10):
        super().__init__()
        self.num_gaussians = num_gaussians
        if num_gaussians > 1:
            self.a_exp = torch.nn.Parameter(
                torch.cat((torch.ones(pos.shape[0], 1), torch.rand(pos.shape[0], num_gaussians - 1)), dim=-1))
            self.c_exp = torch.nn.Parameter(
                torch.cat((torch.ones(pos.shape[0], 1), torch.rand(pos.shape[0], num_gaussians - 1)), dim=-1))
        else:
            self.a_exp = torch.nn.Parameter(torch.ones(pos.shape[0], 1))
            self.c_exp = torch.nn.Parameter(torch.ones(pos.shape[0], 1))

    def forward(self, pos_vn, pos):
        diff = pos_vn.view(-1, 1, 3).expand(-1, pos.shape[0], 3) - pos
        dist = torch.linalg.norm(diff, dim=-1).view(diff.shape[0], diff.shape[1], 1).expand(diff.shape[0],
                                                                                            diff.shape[1],
                                                                                            self.num_gaussians)
        # dist = torch.linalg.norm(diff, dim=-1).repeat(1, self.num_gaussians)
        # print(dist.shape, self.c_exp.shape)
        exp = -0.5 * dist * dist / self.c_exp * self.c_exp
        gaussians_values_all = self.a_exp * torch.exp(exp)
...
@registry.register_model("torchmd_etEarly_vn_gaussian")
class TorchMD_ET(BaseModel):
    ...
    def __init__(
        ...
        self.height_lin_list = nn.ModuleList()
        self.deviation_lin_list = nn.ModuleList()
        for i in range(self.num_post_layers):
            if i == 0:
                self.post_lin_list.append(nn.Linear(hidden_channels, post_hidden_channels))
                self.height_lin_list.append(nn.Linear(hidden_channels, post_hidden_channels))
                self.deviation_lin_list.append(nn.Linear(hidden_channels, post_hidden_channels))
            else:
                self.post_lin_list.append(nn.Linear(post_hidden_channels, post_hidden_channels))
                self.height_lin_list.append(nn.Linear(post_hidden_channels, post_hidden_channels))
                self.deviation_lin_list.append(nn.Linear(post_hidden_channels, post_hidden_channels))

        self.post_lin_list.append(nn.Linear(post_hidden_channels, self.output_dim))
        self.height_lin_list.append(nn.Linear(post_hidden_channels, self.num_gaussians))
        self.deviation_lin_list.append(nn.Linear(post_hidden_channels, self.num_gaussians))

        self.reset_parameters()

    def gaussian(self, a_exp, c_exp, pos_vn, pos):
        diff = pos_vn.view(-1, 1, 3).expand(-1, pos.shape[0], 3) - pos
        dist = torch.linalg.norm(diff, dim=-1).view(diff.shape[0], diff.shape[1], 1).expand(diff.shape[0],
                                                                                            diff.shape[1],
                                                                                            self.num_gaussians)
    ...
    def _forward(self, data):
        ...
        regular_mask = torch.argwhere(data.z != 100).squeeze(1)
        vn_x = torch.index_select(x, 0, virtual_mask)
        a_exp, c_exp = torch.index_select(x, 0, regular_mask), torch.index_select(x, 0, regular_mask)

        for i in range(0, len(self.post_lin_list) - 1):
            vn_x = self.post_lin_list[i](vn_x)
            vn_x = getattr(F, self.activation)(vn_x)
            a_exp = self.height_lin_list[i](a_exp)
            a_exp = getattr(F, self.activation)(a_exp)
            c_exp = self.deviation_lin_list[i](c_exp)
            c_exp = getattr(F, self.activation)(c_exp)

        vn_x = self.post_lin_list[-1](vn_x)
        a_exp = self.height_lin_list[-1](a_exp)
        c_exp = self.deviation_lin_list[-1](c_exp)
