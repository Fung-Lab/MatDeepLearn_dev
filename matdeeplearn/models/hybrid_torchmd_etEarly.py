...
@registry.register_model("hybrid_torchmd_etEarly")


class Hybrid_TorchMD_ET_Early(BaseModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(Hybrid_TorchMD_ET_Early, self).__init__(**kwargs)
    ...
    def _setup_potential_layers(self):
        ...
        for _ in range(self.n_params):
            ...
            for i in range(self.potential_fc_count):
                if i == 0:
                    layer = torch.nn.Linear(self.hidden_channels, self.potential_dim).to('cuda')
                else:
                    layer = torch.nn.Linear(self.potential_dim, self.potential_dim).to('cuda')
                param_list.append(layer)
            out_layer = torch.nn.Linear(self.potential_dim, self.num_elements).to('cuda')
    ...
    def _forward(self, data):
        ...
        # param = [torch.ones((x.shape[0], 100)).to('cuda') for _ in range(3)]
        param = []
        # param layers
        for i in range(len(self.potential_lin_list)):
            param_out = x
            for j in range(len(self.potential_lin_list[i])):
                if j != len(self.potential_lin_list[i]) - 1:
                    ...
                    param_out = self.potential_lin_list[i][j](param_out)
            # if i == 1:
            # param_out = torch.clamp(param_out, min=0, max=10)
            param.append(param_out)
    ...
    def morse_potential(self, params, data):
        adjusted_z = torch.empty_like(data.z)
        adjusted_z[data.z == 8] = 1
        adjusted_z[data.z == 14] = 2
        
        # atoms = data.z[data.edge_index] - 1
        atoms = adjusted_z[data.edge_index] - 1
        
        all_sigmas, all_rms, all_Ds = params[0], params[1], params[2]
        # print(all_sigmas.shape, all_rms.shape, all_Ds.shape)
        # print(all_sigmas[:5, 13], all_rms[:5, 13], torch.nonzero(all_Ds[:, 13]).size(0))
        ...
        # print(D[D > 0], rm[rm < 0], sigma[sigma > 0])
        
        if self.use_atomic_energy:
            base_atomic_energy = torch.zeros((len(self.base_atomic_energy), 1)).to('cuda')
            # for z in np.unique(data.z.cpu()):
            #     base_atomic_energy[z - 1] = self.base_atomic_energy[z - 1]
            for z in np.unique(adjusted_z.cpu()):
                base_atomic_energy[z - 1] = self.base_atomic_energy[z - 1]
        ...
        if self.use_atomic_energy:
            # base_atomic_energy = base_atomic_energy[data.z - 1].squeeze()
            base_atomic_energy = base_atomic_energy[adjusted_z - 1].squeeze()  
            base_atomic_energy = scatter_add(base_atomic_energy, index=data.batch)
