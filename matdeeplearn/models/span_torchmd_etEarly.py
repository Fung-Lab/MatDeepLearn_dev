...
class TorchMD_ET_Early(BaseModel):
    ...
    def _forward(self, data):
        ...
        total_l1_loss = 0
        for attn in self.attention_layers:
            dx, dvec, l1_loss = attn(x, vec, data.edge_index, data.edge_weight, data.edge_attr, data.edge_vec)
            x = x + dx
            vec = vec + dvec
    ...
    def forward(self, data):
        ...
        output = {}
        out, total_l1_loss = self._forward(data)
        output["output"] =  out
        output["l1_loss"] = total_l1_loss
...
class AdaptiveSpan(nn.Module):
    def __init__(self, attn_span, nb_heads, R=32, init_val=0):
        super().__init__()
        self.attn_span = attn_span
        self.R = R
        self.span_params = nn.Parameter(torch.zeros(nb_heads))
        self.span_params.data.fill_(init_val)

    def forward(self, attn):
        z = self.attn_span * torch.sigmoid(self.span_params)
        mask = ((self.R + z - torch.arange(attn.size(0), device=attn.device).float().unsqueeze(1)) / self.R).clamp(0, 1)
        return attn * mask, mask
    
    def get_span_l1(self):
        # Compute the L1 regularization term
        return torch.sum(self.attn_span * torch.sigmoid(self.span_params)) / self.nb_heads
...
class EquivariantMultiHeadAttention(MessagePassing):
    def __init__(
        ...
        self.adaptive_span = AdaptiveSpan(adaptive_span, num_heads)
        self.attn_act = activation()
        self.span_reg = span_reg
    ...
    def forward(self, x, vec, edge_index, r_ij, f_ij, d_ij):
        ...
        return dx, dvec, self.adaptive_span.get_span_l1() * self.span_reg
    ...
    def message(self, q_i, k_j, v_j, vec_j, dk, dv, r_ij, d_ij):
        ...
        attn, mask = self.adaptive_span(attn)
