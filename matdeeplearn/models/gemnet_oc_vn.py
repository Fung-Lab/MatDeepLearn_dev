...
from matdeeplearn.models.layers.atom_update_block import OutputBlock
...
class GemNetOC(BaseModel):
    ...
    def __init__(
        ...
        emb_size_trip_in: int = 128,
        emb_size_trip_out: int = 64,
        emb_size_quad_in: int = 64,
        emb_size_quad_out: int = 64,
        ...
        for i in range(self.num_post_layers):
            if i == 0:
                self.post_lin_list.append(
                    nn.Linear(emb_size_atom * self.max_z, post_hidden_channels)
                )
            else:
                self.post_lin_list.append(
    ...
    def forward(self, data):
        ...
        # combine edges from desired interaction types
        ...
        if self.virtual_node_pool is not None:
            x = self.virtual_node_pool(data, x_E)
        else:
        ...
        if self.regress_forces:
            raise NotImplementedError("Forces not correctly ported over yet")
            # if self.direct_forces:
