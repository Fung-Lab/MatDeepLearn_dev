...
class MorseParameters(nn.Module):
    ...
    def __init__(
        ...
        # Atomic parameters
        for param in ('base_atomic_energy',):
            params = [
                torch.nn.Parameter(
                    torch.full(
                        (dimensions[param],), 1, dtype=torch.float32, device=device
        ...
        if init_mode == 'constant':
            ...
            for key, val_dict in init_args.items():
                ...
                if '-' in key:
                    ...
                    valid_params = ('D', 'rm', 'alpha')
                else:
                    z = self.type_to_idx[key]
                    param_idx = self.z2idx[z]
                    valid_params = ('base_atomic_energy',)
    ...
    def query_batch(self, query: torch.Tensor, param_name: str) -> torch.Tensor:
        param_tensor = torch.zeros(
            (len(self.param_dict[param_name]), 1)
        ).to(self.device)
...
@registry.register_model("Morse_Interaction")
class Morse_Interaction(BaseModel):
    def __init__(
        self,
        atom_types: list[str],
        init_mode: Literal['constant', 'covalent_radius', 'custom'] = 'constant',
        ...
        **kwargs
        ...
        super(Morse_Interaction, self).__init__(**kwargs)
        if init_args is None:
            init_args = {}
        self.morse_params = MorseParameters(
            atom_types, init_mode, init_args, freeze_params
        )
    ...
    def morse_potential(self, data):
        ...
        D = self.morse_params.query_batch(atoms, 'D')
        rm = self.morse_params.query_batch(atoms, 'rm')
        alpha = self.morse_params.query_batch(atoms, 'alpha')
        base_atomic_energy = self.morse_params.query_batch(data.z, 'base_atomic_energy')
