...
@registry.register_model("CGCNN_VN_OLD")
class CGCNN_VN_OLD(BaseModel):
    def __init__(
        self,
        edge_steps,
        self_loop,
        ...
        **kwargs
        ...
        super(CGCNN_VN_OLD, self).__init__(edge_steps, self_loop)
            ...
            for i in range(self.post_fc_count):
                ...
                if i == 0:
                    ...
                    if self.pool_order == "early" and (
                        ...
                        lin = torch.nn.Linear(self.post_fc_dim * 100, self.dim2)
                    else:
    ...
    def forward(self, data: Data):
        ...
        for i in range(0, len(self.pre_lin_list)):
            if i == 0:
                out = self.pre_lin_list[i](data.x_both.float())
            ...
            if len(self.pre_lin_list) == 0 and i == 0:
                if self.batch_norm:
                    out = self.conv_list[i](
                        data.x_both, data.edge_index_both, data.edge_attr_both.float()
                    )
                ...
                if self.batch_norm:
                    out = self.conv_list[i](
                        out, data.edge_index_both, data.edge_attr_both.float()
                    )
                    out = self.bn_list[i](out)
                else:
                    out = self.conv_list[i](
