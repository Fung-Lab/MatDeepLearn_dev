...
class Morse(BaseModel):
    def __init__(
        ...
        **kwargs
        ...
        self.alphas = Embedding(100, 1).to('cuda:0')
        self.D = Embedding(100, 1).to('cuda:0')
        self.base_atomic_energy = Embedding(100, 1).to('cuda:0')
        
        nn.init.constant_(self.rm.weight, 3)
        nn.init.constant_(self.alphas.weight, 3)
        nn.init.constant_(self.D.weight, 3)
        nn.init.constant_(self.base_atomic_energy.weight, -1.5)
        
        self.alphas.weight.requires_grad = False
    ...
    def morse_potential(self, data):
        ...
        alpha_i, alpha_j = self.alphas(atoms[0]), self.alphas(atoms[1])
        D_i, D_j = self.D(atoms[0]), self.D(atoms[1])

        rm = (rm_i + rm_j).squeeze() / 2
        alpha = (alpha_i + alpha_j).squeeze() / 2
        D = (D_i + D_j).squeeze() / 2
