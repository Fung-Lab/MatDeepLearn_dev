    ...
    def __init__(
        ...
        **kwargs
        ...
        if use_distill:
            ...
            ), "Distill layer number in SchNet is less than 1"
            assert all(
                code[1] <= (pre_fc_count+gc_count+post_fc_count) for code in distill_layers
            ), "Distill layer number in SchNet is large than num_layers"
            assert all(
                code[0] in ["pr", "g", "po"] for code in distill_layers
            ), "Distill layer type in SchNet not pr(pre_fc), g(gc), po(post_fc)"
