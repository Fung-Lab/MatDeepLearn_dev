...
from torch.nn import Sequential, SiLU, Sigmoid
...
# TODO: the memory usage spike is occurring here, figure out what is underlying cause to allow training
...
class NodeModel(torch.nn.Module):
    ...
    def __init__(self, input_features, output_features, residual=True, eps=1e-6) -> None:
        super().__init__()
        # sam
...
class EdgeGatedGraphConvNoMP(torch.nn.Module):
    ...
    def __init__(self, input_features: int, output_features: int, residual: bool = True) -> None:
        super().__init__()
...
class ALIGNNConv(torch.nn.Module):
    ...
    def forward(self, g: Data, node_feats: torch.Tensor, edge_attr: torch.Tensor, triplet_feats: torch.Tensor) -> torch.Tensor:
...
class ALIGNN(BaseModel):
    def __init__(
        ...
        **kwargs
        ...
        self.atom_embedding = EmbeddingLayer(
            atom_input_features, hidden_features
        )

        self.edge_embedding = torch.nn.Sequential(
            RBFExpansion(vmin=min_edge_distance,
                         vmax=max_edge_distance, bins=edge_input_features),
        ...
        self.angle_embedding = torch.nn.Sequential(
            RBFExpansion(vmin=min_angle, vmax=max_angle,
