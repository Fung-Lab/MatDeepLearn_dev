...
def split_heads(x, n):
    return torch.transpose(split_states(x, n), -2, -1)

def merge_heads(x):
    return merge_states(torch.transpose(x, -2, -1))
...
def attention_impl(q, k, v, heads, attn_mode, local_attn_ctx=None, attn_bias=None):
...
def blocksparse_attention_impl(q, k, v, heads, attn_mode, local_attn_ctx=None, blocksize=32, attn_bias=None):
    n_ctx = q.size()[1]
    ...
    w = torch.matmul(q, k.transpose(-2, -1)) + attn_bias
...
class SparseAttention(nn.Module):
    ...
    def forward(self, q, k, v, attn_bias=None):
        return blocksparse_attention_impl(q, k, v, self.heads, self.attn_mode, self.local_attn_ctx, self.blocksize, attn_bias=attn_bias)
