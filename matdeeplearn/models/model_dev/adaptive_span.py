...
class AdaptiveSpan(nn.Module):
    ...
    def trim_memory(self, query, key, value, key_pe, attn_bias=None):
        ...
        if attn_bias is not None:
            n_head, n_node, _ = attn_bias.size()
            if -trim_len_cache == n_node:
                pass
            attn_bias = attn_bias[:, :, trim_len_cache-1:]
            if attn_bias.size(2) <= -trim_len_cache:
                attn_bias = F.pad(attn_bias, [0, -trim_len_cache-attn_bias.size(2)+1])
            attn_bias = attn_bias.view(n_head * n_node, -1, -(trim_len_cache - 1))
            
        return key, value, key_pe, attn_bias
