...
class MPNN(BaseModel):
    def __init__(
        ...
        **kwargs
        ...
        super(MPNN, self).__init__(edge_steps, self_loop)
        ...
        if pre_fc_count == 0:
            self.gc_dim, self.post_fc_dim = self.data.num_features, data.num_features
        else:
    ...
    def _setup_gnn_layers(self):
        ...
        for i in range(self.gc_count):
            nn = Sequential(
                Linear(self.num_edge_features, self.dim3),
                ReLU(),
