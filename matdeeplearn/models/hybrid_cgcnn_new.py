...
@registry.register_model("Hybrid_CGCNN_New")
class Hybrid_CGCNN_New(BaseModel):
    def __init__(
        self,
        node_dim,
        edge_dim,
        ...
        **kwargs
        ...
        super(Hybrid_CGCNN_New, self).__init__(**kwargs)
        ...
        for i in range(len(self.potential_lin_list)):
            ...
            for j in range(len(self.potential_lin_list[i])):
                if j != len(self.potential_lin_list[i]) - 1:
                    ...
                    param_out = self.potential_lin_list[i][j](param_out)
            # if i == 1:
            # param_out = torch.clamp(param_out, min=1.5, max=10)    
