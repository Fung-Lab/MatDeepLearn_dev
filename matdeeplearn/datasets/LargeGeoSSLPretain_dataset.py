...
class LargeDataProcessor:
    ...
    def process(self, dict_structures, perturb=False, mu=0, sigma=0.3):

        data_list = self.get_data_list(dict_structures, perturb, mu, sigma)
        data, slices = InMemoryDataset.collate(data_list)
        return data

    def get_data_list(self, dict_structures, perturb, mu, sigma):
        ...
        for i, sdict in enumerate(dict_structures):
            ...
            if perturb:
                pos = pos + torch.normal(mu, sigma, size=pos.size())
                # print("After perturbing: ", pos)
        ...
        if self.preprocess_nodes == True:
            ...
            generate_node_features(data_list_1, self.n_neighbors, device=self.device)

        if self.preprocess_edge_features == True:
            # logging.info("Generating edge features...")
            generate_edge_features(data_list_1, self.edge_steps, self.r, device=self.device)
...
class LargeGeoSSlPretrainDataset(InMemoryDataset):
    def __init__(
            self,
            root,
            processed_data_path,
        ...
        super(LargeGeoSSlPretrainDataset, self).__init__(
    ...
    def __getitem__(self, idx):
        ...
        subdata1 = self.processer.process([self.dict_structures[idx]], perturb=False)
...
if __name__ == '__main__':
    ...
    dataset = LargeGeoSSlPretrainDataset(root="data/ct_pretrain/data.json", processed_data_path="",augmentation_list=config["dataset"]["augmentation"],
