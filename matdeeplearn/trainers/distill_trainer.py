...
class DistillForcesTrainer(BaseTrainer):
    ...
    def from_config(cls, config):
        ...
        model.set_teacher_dim(teacher_dim)
        model.setup_distillation()
        model.to(rank)
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, end_epoch):            
            ...
            for i in pbar:                                
                ...
                _metrics = self._compute_distill_metrics(out, batch, _metrics)               
                self.metrics = self.evaluator.update("loss", loss.item(), out["s_out"]["output"].shape[0], _metrics)     
    ...
    def validate(self, split="val"):
        ...
        for i in range(0, len(loader_iter)):
            ...
            # Compute metrics
            #print(i, torch.cuda.memory_allocated() / (1024 * 1024), torch.cuda.memory_cached() / (1024 * 1024))          
            metrics = self._compute_distill_metrics(out, batch, metrics)
            metrics = evaluator.update("loss", loss.item(), out["s_out"]["output"].shape[0], metrics)
            del loss, batch, out
    ...
    def predict(self, loader, split, results_dir="train_results", write_output=True, labels=True):        
        ...
        for i in range(0, len(loader_iter)):
            batch = next(loader_iter).to(self.rank)      
            out = self._distill_forward(batch.to(self.rank))
            out = out["s_out"]
