...
class DistillForcesTrainer(BaseTrainer):
    ...
    def from_config(cls, config):
        ...
        model[0].set_teacher_dim(teacher_dim)
        model[0].setup_distillation()
        model[0].to(rank)
        ...
        loss = cls._load_loss(config["optim"]["loss"], preprocess_teacher_features)
        ...
        original_loss_fn = cls._load_original_loss(config["optim"]["loss"])
    ...
    def _load_teacher_model(teacher_config, graph_config, dataset, world_size, rank):
        teacher_model = DistillForcesTrainer._load_model(teacher_config, graph_config, dataset, world_size, rank)[0]
        if not teacher_config['trainable']:
            if not teacher_config['teacher_checkpoint_path']:
                raise ValueError("No checkpoint directory specified in teacher config.")

            # Load teacher model parameters
            checkpoint = torch.load(teacher_config['teacher_checkpoint_path'])
    ...
    def _load_loss(loss_config, preprocess_teacher_features):
        ...
        for arg in optional_args:
            if arg in loss_config:
                loss_args[arg] = loss_config[arg]
        
        loss_args['preprocess_teacher_features'] = preprocess_teacher_features
        return loss_cls(**loss_args)

    @staticmethod
    def _load_original_loss(loss_config):
        """Loads the original loss for predict method"""
        original_loss_fn = registry.get_loss_class("ForceStressLoss")
        ...
        original_loss_fn = original_loss_fn(**loss_args)
        ...
        for epoch in range(start_epoch, end_epoch):            
            ...
            for i in pbar:                                
                ...
                with autocast(enabled=self.use_amp):
                    out_list = self._distill_forward(batch)                                            
                    if self.attention_weight:
                        out_list = self._compute_attention(out_list, batch)
                    loss = self._compute_distill_loss(out_list, batch)  
                #print(i, torch.cuda.memory_allocated() / (1024 * 1024), torch.cuda.memory_cached() / (1024 * 1024))                                               
                grad_norm = []
                for i in range(len(self.model)):
                ...
                for n in range(len(self.model)):
                    _metrics[n] = self._compute_distill_metrics(out_list[n], batch[n], _metrics[n])
                    self.metrics[n] = self.evaluator.update("loss", loss[n].item(), out_list[n]["s_out"]["output"].shape[0], _metrics[n])
    ...
    def validate(self, split="val"):
        ...
        for i in range(0, len(loader_iter[0])):
            ...
            out_list = self._distill_forward(batch)
            if self.attention_weight:
                out = self._compute_attention(out_list, batch)
            loss = self._compute_distill_loss(out_list, batch)
            # Compute metrics
            #print(i, torch.cuda.memory_allocated() / (1024 * 1024), torch.cuda.memory_cached() / (1024 * 1024))          
            for n in range(len(self.model)):
                metrics[n] = self._compute_distill_metrics(out_list[n], batch[n], metrics[n])
                metrics[n] = evaluator.update("loss", loss[n].item(), out_list[n]["s_out"]["output"].shape[0], metrics[n])    
    ...
    def predict(self, loader, split, results_dir="train_results", write_output=True, labels=True):        
        ...
        for i in range(0, len(loader_iter)):
            ...
            batch_p = [o["s_out"]["output"].data.cpu().numpy() for o in out_list]
        ...
        if labels == True:
            predict_loss = metrics[type(self.original_loss_fn).__name__]["metric"]
    ...
    def _distill_forward(self, batch_list):
        ...
        if self.preprocess_teacher_features:
            s_out = self.model[0].extract_feature(batch_list[0])
            return [{"s_out": s_out}]
        t_out = self.teacher_model.extract_feature(batch_list[0])
        s_out = self.model[0].extract_feature(batch_list[0])
        return [{"s_out": s_out, "t_out": t_out}]
    
    def _compute_attention(self, output, batch_list):
        if self.preprocess_teacher_features:
            output = self.attention_model.compute_attention(output, batch_list[0])
        else:
    ...
    def _compute_loss(self, out, batch_data):
        if isinstance(out, list):
            ...
            for i in range(len(out)):
                loss.append(self.original_loss_fn(out[i], batch_data[i]))
        else:
            loss = self.original_loss_fn(out, batch_data)
        return loss

    def _compute_distill_loss(self, out, batch_data):
        if isinstance(out, list):
            loss = []
            for i in range(len(out)):
    ...
    def _compute_metrics(self, out, batch_data, metrics):
        ...
        metrics = self.evaluator.eval(
            out, property_target, self.original_loss_fn, prev_metrics=metrics
	    )

        return metrics
    
    def _compute_distill_metrics(self, out, batch_data, metrics):
        try:
            property_target = batch_data.to(self.rank)
        except:
            property_target = batch_data
