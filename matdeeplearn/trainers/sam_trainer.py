...
from matdeeplearn.trainers.sam import SAM


@registry.register_trainer("sam")
class SamTrainer(BaseTrainer):
    ...
    def from_config(cls, config):
        ...
        trainer_cls.optimizer = optim_list
        return trainer_cls
        ...
        for epoch in range(start_epoch, end_epoch):            
            ...
            for i in pbar:                                
                ...
                with autocast(enabled=self.use_amp):
                    out_list, loss, grad_norm = self._sam_backward(batch)
    ...
    def _sam_backward(self, batch_data):
        def closure():
            loss_list = self._compute_loss(self._forward(batch_data), batch_data)
            for i in range(len(loss_list)):
                self.optimizer[i].zero_grad(set_to_none=True)
                self.scaler.scale(loss_list[i]).backward(retain_graph=True)
            return loss_list

        out_list = self._forward(batch_data)
        loss_list = self._compute_loss(out_list, batch_data)
        grad_norm_list = []        
        for i in range(len(loss_list)):
            self.optimizer[i].zero_grad(set_to_none=True)
            self.scaler.scale(loss_list[i]).backward(retain_graph=True)
            ...
            self.scaler.step(self.optimizer[i], closure)
            self.scaler.update()
        
        return out_list, loss_list, grad_norm_list
