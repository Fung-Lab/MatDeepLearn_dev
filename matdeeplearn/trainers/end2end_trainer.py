...
@registry.register_trainer("end2end")
class PropertyTrainer(BaseTrainer):
    ...
    def train(self):
        ...
        print("No. of batches: ", len(self.data_loader["train_loader"]))
    ...
    def get_end2end_loss(self, batch, target_rep):
        """
        ...
        init_pos = torch.rand(batch.pos.shape, device=self.rank)
        batch.pos = init_pos

        # set gradient to be false to avoid pass through the model twice
        self.model.gradient = False
        batch.pos, _ = self.position_optimizer.optimize(batch, target_rep)
