...
class PropertyTrainer(BaseTrainer):
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, end_epoch):
            ...
            for i in range(len(self.model)):
                train_loader_iter.append(
                    iter(self.data_loader[i]["train_loader"]))
            # metrics for every epoch
            _metrics = [{} for _ in range(len(self.model))]

            # for i in range(skip_steps, len(self.train_loader)):
            pbar = tqdm(
                range(0, len(self.data_loader[0]["train_loader"])), disable=not self.batch_tqdm)
            for i in pbar:
                ...
                pbar.set_description("Batch Loss {:.4f}, grad norm {:.4f}".format(torch.mean(
                    torch.stack(loss)).item(), torch.mean(torch.stack(grad_norm)).item()))
                # Compute metrics
                ...
                for n in range(len(self.model)):
                    _metrics[n] = self._compute_metrics(
                        out_list[n], batch[n], _metrics[n])
                    self.metrics[n] = self.evaluator.update(
                        ...
                        if self.output_frequency == 0:
                            if self.model_save_frequency == 1:
                                self.update_best_model(
                                    metric[i], i, write_model=True, write_csv=False)
                            else:
                                self.update_best_model(
                                    metric[i], i, write_model=False, write_csv=False)
                        elif self.output_frequency == 1:
                            if self.model_save_frequency == 1:
                                self.update_best_model(
                                    metric[i], i, write_model=True, write_csv=True)
                            else:
                                self.update_best_model(
                                    metric[i], i, write_model=False, write_csv=True)
        ...
        if self.best_model_state:
            ...
            #    test_loss = "N/A"
            if self.model_save_frequency != -1:
                self.save_model("best_checkpoint.pt", index=None,
                                metric=metric, training_state=True)
    ...
    def validate(self, split="val"):
        ...
        for i in range(0, len(loader_iter[0])):
            ...
            # print(i, torch.cuda.memory_allocated() / (1024 * 1024), torch.cuda.memory_cached() / (1024 * 1024))
            for n in range(len(self.model)):
                metrics[n] = self._compute_metrics(
                    out_list[n], batch[n], metrics[n])
                metrics[n] = evaluator.update(
                    "loss", loss[n].item(), out_list[n]["output"].shape[0], metrics[n])
    ...
    def predict(self, loader, split, results_dir="train_results", write_output=True, labels=True):
        ...
        for i in range(0, len(loader_iter)):
            ...
            if labels == True:
                ...
                if str(self.rank) not in ("cpu", "cuda"):
                    batch_t = batch[self.model[0].module.target_attr].cpu(
                    ).numpy()
                else:
            ...
            if out.get("pos_grad") != None:
                ...
                batch_ids_pos_grad = np.column_stack(
                    (structure_ids_pos_grad, node_ids_pos_grad))
                ...
                predict_pos_grad_std = batch_p_pos_grad_std if i == 0 else np.concatenate(
                    (predict_pos_grad_std, batch_p_pos_grad_std), axis=0)
                if "forces" in batch:
            ...
            if out.get("cell_grad") != None:
                batch_p_cell_grad = out["cell_grad"].data.view(
                    out["cell_grad"].data.size(0), -1).cpu().numpy()
                batch_p_cell_grad_std = out["cell_grad_std"].data.view(
                    out["cell_grad"].data.size(0), -1).cpu().numpy()
                ...
                predict_cell_grad_std = batch_p_cell_grad_std if i == 0 else np.concatenate(
                    (predict_cell_grad_std, batch_p_cell_grad_std), axis=0)
                if "stress" in batch:
                    batch_t_cell_grad = batch["stress"].view(
                        out["cell_grad"].data.size(0), -1).cpu().numpy()
            ...
            predict_mean = batch_p_mean if i == 0 else np.concatenate(
                (predict_mean, batch_p_mean), axis=0)
            ...
            for x in range(len(self.model)):
                predict[x] = batch_p[x] if i == 0 else np.concatenate(
                    (predict[x], batch_p[x]), axis=0)

            if labels == True:
                target = batch_t if i == 0 else np.concatenate(
                    (target, batch_t), axis=0)
        ...
        if labels == True:
            ...
            logging.info("Saved {:s} error: {:.5f}".format(
                split, predict_loss))
            if len(self.model) > 1:
                predictions = {"ids": ids, "predict": predict_mean,
                               "target": target, "std": stds}
            else:
                predictions = {"ids": ids,
                               "predict": predict_mean, "target": target}
        else:
            if len(self.model) > 1:
                predictions = {"ids": ids,
                               "predict": predict_mean, "std": stds}
            else:
                predictions = {"ids": ids, "predict": predict_mean}
    ...
    def predict_by_calculator(self, loader):
        ...
        assert isinstance(loader, torch.utils.data.dataloader.DataLoader)
        assert len(
            loader) == 1, f"Predicting by calculator only allows one structure at a time, but got {len(loader)} structures."
        ...
        for i in range(0, len(loader_iter)):
            ...
            energy = None if out.get('output') is None else out.get(
                'output').data.cpu().numpy()
            stress = None if out.get('cell_grad') is None else out.get(
                'cell_grad').view(-1, 3).data.cpu().numpy()
            forces = None if out.get('pos_grad') is None else out.get(
                'pos_grad').data.cpu().numpy()
    ...
    def _log_metrics(self, val_metrics=None):
        train_loss = [torch.tensor(
            i[type(self.loss_fn).__name__]["metric"]) for i in self.metrics]
        ...
        if not val_metrics:
            ...
            val_loss = [torch.tensor(
                i[type(self.loss_fn).__name__]["metric"]) for i in val_metrics]
