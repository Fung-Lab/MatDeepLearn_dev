...
@registry.register_trainer("ct_pretrain_sim")
        ...
        for epoch in range(start_epoch, end_epoch):
            ...
            if self.val_loader:
                ...
                if epoch % self.train_verbosity == 0:
                    logging.info(
                        "Epoch: {:04d}, Learning Rate: {:.6f}, Training Error: {:.5f}, Val "
                        "Error: {:.5f}, Time per epoch (s): {:.5f}".format(
                            int(self.epoch - 1),
                            self.scheduler.lr,
                ...
                if val_loss < self.best_metric:
                    ...
                    self.save_model("best_checkpoint.pt", val_loss, False)
    ...
    def validate(self, split="val"):
        ...
        for i in range(0, len(loader_iter)):
            with torch.no_grad():
                ...
                p1, z1 = self._forward(batch1.to(self.device))
                p2, z2 = self._forward(batch2.to(self.device))
                loss_total += self._compute_loss(p1, z1, p2, z2) * len(batch1)
                total_num += len(batch1)
        torch.cuda.empty_cache()
