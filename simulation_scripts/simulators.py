...
class MetricMDSimulator(AbstractSimulator):
    ...
    def __init__(self, config_path: str):
        ...
        self.has_alarm = 'alarms' in config.keys()
        if self.has_alarm:
            alarms = config['alarms']
            self.energy_alarm = alarms.get('energy_diff', 0.2)
            self.rdf_alarm = alarms.get('rdf_diff', 2)
            self.min_dist_alarm = alarms.get('min_dist', 1.3)
            self.max_dist_alarm = alarms.get('max_dist', 4.0)

            self.large_force_alarm = alarms.get('large_force_num', self.total_steps // 10)
            logging.info(f"Alarms set to: energy_diff={self.energy_alarm:.3f}, rdf_diff={self.rdf_alarm:.3f}, min_dist_diff={self.min_dist_alarm:.2f}, large_force_num={self.large_force_alarm}")
    ...
    def update_rolling_rdf(self, rolling_rdf, current_rdf, final_metrics, window_size):
        if rolling_rdf.shape[0] != window_size:
            ...
            rdf_change = np.mean(np.abs(current_rdf - mean_prev_rdf))
            final_metrics[f'highest_rdf_mae_{window_size}'] = max(final_metrics[f'highest_rdf_mae_{window_size}'], rdf_change)
            rolling_rdf[0] = current_rdf
            rolling_rdf = np.roll(rolling_rdf, -1, axis=0)
        return rolling_rdf
    ...
    def run_simulation(self,
        ...
        if 'mae_rdf' in self.metrics:
            ...
            final_metrics['highest_rdf_mae_2'] = 0
            final_metrics['highest_rdf_mae_500'] = 0
        ...
        def update_metics(a=atoms):
            nonlocal final_metrics, metrics, rolling_rdf_2, rolling_rdf_500, save_traj
            if 'energy' in self.metrics:
                ...
                if self.has_alarm and not save_traj and\
                    (final_metrics['highest_e'] < final_metrics['starting_energy'] * (1 + self.energy_alarm) or\
                    final_metrics['lowest_e'] > final_metrics['starting_energy'] * (1 - self.energy_alarm)):
                    save_traj = True
                    print(final_metrics['starting_energy'] * (1 + self.energy_alarm), final_metrics['starting_energy'] * (1 - self.energy_alarm))
                    logging.info(f"Alarm triggered for energy={etotal}.")
            if 'mae_rdf' in self.metrics:
                current_rdf = self.get_rdf(atoms)
            ...
            if 'min_dist' in self.metrics:
                ...
                if self.has_alarm and not save_traj and\
                    (final_metrics['max_min_dist'] > self.max_dist_alarm or\
                    final_metrics['min_min_dist'] < self.min_dist_alarm):
                    save_traj = True
                    print(final_metrics['min_min_dist'], self.min_dist_alarm, self.max_dist_alarm)
                    logging.info(f"Alarm triggered for min distance={curr_min_dist}.")
            if 'large_forces' in self.metrics:
                ...
                if self.has_alarm and not save_traj and\
                    final_metrics['large_forces'] > self.large_force_alarm:
                    save_traj = True
                    logging.info(f"Alarm triggered for {final_metrics['large_forces']} large forces.")
